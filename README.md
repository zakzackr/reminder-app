...
#### ブラウザのセッションストレージを使用したセッション管理
ユーザーがログインすると、システムはユーザー認証を行い、データベースから該当するユーザーIDを取得します。このユーザーIDは、HTTPレスポンスのボディ内でクライアントに返されます。</br>
クライアント側で、送られてきたユーザーIDを、ブラウザのセッションストレージに格納します。これにより、ユーザーがブラウザのタブを閉じるまで、ユーザーはセッションストレージに保存したユーザーIDを使用して、HTTPリクエストを送信します。具体的には、ユーザーがリマインダーを追加・更新する際、セッションストレージから取得したユーザーIDをHTTPリクエストのURLに含めて送信します。これにより、各ユーザーに対応するリマインダーを正確に管理することができます。<br>
...
<br>  
#### @Scheduledの使用
@Scheduledを使用して、リマインドするタスクを探索する関数を定期的に実行します。そして、現在の時刻と通知時間が一致しているタスクをデータベースから取得して、そのユーザー宛にメールを送信します。Reminderテーブル内で、外部キーとしてユーザーIDを保持することで、リマインドするタスクとその受信者を結びつけることで、リマインダーを設定したユーザーにメールを送信することが可能になります。
<br>  
