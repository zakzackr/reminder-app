...
#### Spring Securityによるログイン機能の導入
ユーザーがログインすると、システムはユーザー認証を行い、JWTトークンやユーザーIDなどの情報を持つJwtAuthResponseオブジェクトをHTTPレスポンスでクライアントに送信します。
...
<br>
具体的には、ユーザーがリマインダーに対して操作を行う際、セッションストレージから取り出したユーザーIDを以下のようにHTTPリクエストのURLに含めます。これにより、他のユーザーとリマインダーが混在することなく、各ユーザー専用のリマインダーを設定・管理することが可能です
　
| 操作 |HTTPメソッド| URL |
| ---- | ---- |---|
